      ...
      nodeSelectorTerms:
        - matchExpressions:
            ...
            - key: kubernetes.io/os
              ...
              values:
                - linux
  podAntiAffinity:                 # 硬性打散：同组件副本不共节点
    requiredDuringSchedulingIgnoredDuringExecution:
      - labelSelector:
          matchExpressions:
...
> 反亲和按组件隔离（应用 `server`、Casdoor `casdoor`），两者**可同节点**，
> 因此**带 `flyiam=true` 标签的节点数需 ≥ max(应用副本数, Casdoor 副本数)**。
...
> 管理员用户名/邮箱、Casdoor 同步等存于数据库（**DB 优先 / env 兜底**），
> 在「系统设置」页面修改后保存即生效；**Casdoor 连接**支持热重载、无需重启。
> 上表仅保留登录与首次自动初始化必需的引导项。

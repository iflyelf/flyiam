...
--
-- 说明：
--   1. 本系统所有表由程序启动时自动创建（CREATE TABLE IF NOT EXISTS），
--      无需手工执行本文件；此处仅作为表结构参考与审计用途。
--   2. Casdoor 表（前缀 casdoor_）由 Casdoor 首次启动自动创建，不在此列。
--   3. 数据库采用「一库两域」：同一库内包含 Casdoor 表与本系统业务表。
--   4. 需管理员预先创建空库：
--        CREATE DATABASE flyiam;
--     建库后启动 FlyIAM 与 Casdoor 即自动建表。
-- =============================================================================
